...
func Hand_shake(r *bufio.Reader, conn net.Conn) error    {
	...
	fmt.Printf("版本号是:%d",versiom) //解析版本
	if versiom != 5 {
...
func Read_Addr(r *bufio.Reader) (string ,error) {
    ...
    */
	...
	fmt.Printf("客户端请求的远程服务器地址类型是:%d",addrtype) /*“addrtype”代表请求的远程服务器地址类型，它是一个可变参数，但是它值的长度1个字节，
    有三种类型：
...
func handle_conn(conn net.Conn) {
	...
	fmt.Println("得到的完整的地址是：",addr) //注意：HTTP对应的是80端口，HTTPS对应的是443端口。
	resp := []byte{0x05, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00} //详情请参考：http://www.cnbfmts.com/yinzhengjie/p/7357860.html
